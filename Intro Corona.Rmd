...
df<-df_raw %>% 
  add_column(today) %>% 
  select(today,`Total Confirmed`,` Total Fatalities`) %>% 
  rename(Date=today)
df %>%
  kable() %>%
  kable_styling(position = "left",full_width = F,bootstrap_options = "striped", font_size = 22) %>% 
  row_spec(0, bold = T)
...
### New recorded cases last 24 hours.
```{r new cases confirmed,fig.cap = " Top 15 countries with most new cases of covid-19", echo=FALSE,warning=FALSE,message=FALSE}

new_cases(today,ncovConfirmedTidy,"Confirmed")
...
Reported fatalities related to Covid-19, last 24 hours

```{r new cases dead,fig.cap = " Top 15 countries with most recorded fatalities of covid-19 last 24h", echo=FALSE,warning=FALSE,message=FALSE}

new_cases(today,ncovDeathsTidy,"dead")
...
<!-- Reported fatalities related to Covid-19, last 24 hours, pr capita -->
<!-- ```{r new cases fatalities pop,fig.cap = " Top 15 countries with most new fatalities pr capita of covid-19", echo=FALSE,warning=FALSE,message=FALSE} -->
<!-- #sick_dead_date<-"2020-03-25" -->
<!-- sick_dead_date<-today -->

<!-- scandic<-c("Norway", "Sweden", "Denmark", "Finland", "United Kingdom", "Germany") -->
<!-- countries_sick<-scandic -->
<!-- countries_sick<- ncovConfirmedTidy %>%  -->
<!--   filter(date==sick_dead_date) %>%  -->
<!--   group_by(Country.Region) %>%  -->
<!--   summarize(total=sum(value)) %>%  -->
...
<!-- conf_dead_rec<-ncovConfirmedTidy %>%  -->
<!--   filter(value!=0, Country.Region!="Cruise Ship", date<sick_dead_date) %>% -->
<!--   group_by(Country.Region,date) %>% -->
<!--   summarize(confirmed=sum(value)) %>%  -->
<!--   left_join(death_compare) %>%  -->
...
<!-- scandic<-c("Norway", "Sweden", "Denmark", "Finland", "United Kingdom", "Germany", "France") -->

<!-- #Plot dead and sick Curve -->
<!-- plot_sick<-conf_dead_rec %>%  -->
